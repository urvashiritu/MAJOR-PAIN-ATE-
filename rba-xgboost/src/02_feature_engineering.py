...
def feature_sql(src: str) -> str:
    ...
    Features include 14 original + 5 temporal:
      Original: hour, is_night, is_weekend, country_change, device_change,
                failed_recently, rapid_login_rate, login_frequency_today,
                ip/country/asn/device/os/browser_seen_before
      Temporal: hours_since_last_login, login_frequency_24h,
    ...
    SELECT p.*,
        ...
        EXTRACT(EPOCH FROM (p.ts - LAG(p.ts) OVER w)) / 3600.0 AS hours_since_last_login,
        COUNT(*) OVER (
            PARTITION BY p.user_id ORDER BY p.ts
            RANGE BETWEEN INTERVAL '24 hours' PRECEDING AND CURRENT ROW
            EXCLUDE CURRENT ROW) AS login_frequency_24h,
        ABS(EXTRACT(HOUR FROM p.ts) - COALESCE(uh.median_hour, 12)) AS hour_deviation,
        COUNT(DISTINCT p.ip) OVER (
            PARTITION BY p.user_id ORDER BY p.ts
            RANGE BETWEEN INTERVAL '7 days' PRECEDING AND CURRENT ROW
            EXCLUDE CURRENT ROW) AS unique_ips_7d,
        CASE WHEN p.rn = 1 THEN FALSE
             ELSE CASE
                 WHEN LAG(p.country) OVER w IS NULL OR p.country = LAG(p.country) OVER w THEN FALSE
                 ELSE TRUE
             END
        END AS impossible_travel
    FROM prior_fail p
    LEFT JOIN (SELECT user_id, median_hour FROM user_hours GROUP BY user_id, median_hour) uh
        ON p.user_id = uh.user_id
...
def run_gates(con: duckdb.DuckDBPyConnection, out: Path, expected_rows: int) -> list:
    ...
    row = con.execute(f"""
        SELECT COUNT(*) AS n,
               ...
               COUNT(*) FILTER (WHERE hours_since_last_login IS NULL AND rn > 1) AS null_hsl,
               COUNT(*) FILTER (WHERE login_frequency_24h IS NULL) AS null_lf24,
               COUNT(*) FILTER (WHERE hour_deviation IS NULL) AS null_hd,
               COUNT(*) FILTER (WHERE unique_ips_7d IS NULL) AS null_ui7
    ...
    (n, null_hour, null_cc, null_dc, first_cc, first_dc, null_fr, null_rlr, null_lft,
     ...
     first_isb, first_csb, first_ab, first_dsb, first_osb, first_bsb,
     null_hsl, null_lf24, null_hd, null_ui7) = row
    if n != expected_rows:
    ...
    if null_hsl or null_lf24 or null_hd or null_ui7:
        failures.append(f"temporal NULLs: hsl={null_hsl} lf24={null_lf24} hd={null_hd} ui7={null_ui7}")
