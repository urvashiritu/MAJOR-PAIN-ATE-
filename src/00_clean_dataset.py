...
SELECT
    ...
    CASE
        ...
        WHEN regexp_matches(ua, '(?i){IOS_WEBKIT}|{IOS_TOKEN}')
             AND NOT regexp_matches(ua, '(?i){ANDROID_TOKEN}|Windows Phone')
             AND NOT regexp_matches(os_raw, '(?i)(Android|Andorid)|Windows Phone') THEN 'iOS'
        WHEN regexp_matches(ua, '(?i){ANDROID_TOKEN}') THEN 'Android'
        ...
        WHEN regexp_matches(os_raw, '(?i){IOS_WEBKIT}|{IOS_TOKEN}')
        ...
        WHEN regexp_matches(os_raw, '(?i){ANDROID_TOKEN}') THEN 'Android'
    ...
    CASE
        ...
        WHEN regexp_matches(ua, '(?i)iPhone|iPod|Windows Phone|{ANDROID_TOKEN}')
             OR (regexp_matches(ua, '(^|[^A-Za-z0-9])Mobile([^A-Za-z0-9]|$)')
                 AND NOT regexp_matches(ua, '(?i){DESKTOP_OS_MARKER}')) THEN 'mobile'
    ...
    CASE
        WHEN regexp_matches(ua, '(?i){IOS_WEBKIT}|{IOS_TOKEN}')
             ...
             AND regexp_matches(os_raw, '(?i){ANDROID_TOKEN}') THEN TRUE
        WHEN regexp_matches(ua, '(?i){ANDROID_TOKEN}')
             AND regexp_matches(os_raw, '(?i){IOS_WEBKIT}|{IOS_TOKEN}|Mac OS') THEN TRUE
        WHEN regexp_matches(ua, '(?i)Windows')
             AND regexp_matches(os_raw, '(?i){ANDROID_TOKEN}|{IOS_WEBKIT}|{IOS_TOKEN}|Mac') THEN TRUE
        WHEN regexp_matches(ua, '(?i){ANDROID_TOKEN}|{IOS_TOKEN}')
...
CHECKS_CLEAN = {
    ...
    "genbot_as_ios": f"COUNT(*) FILTER (WHERE os_family='iOS' AND ((regexp_matches(user_agent, '(?i)AwarioSmartBot') AND NOT regexp_matches(user_agent, '(?i){IOS_TOKEN}|{IOS_WEBKIT}')) OR (regexp_matches(user_agent, '(?i)CriOS') AND regexp_matches(os_raw, '(?i)Android'))))",
    ...
    "desktop_reclass_mobile": f"COUNT(*) FILTER (WHERE device_raw='desktop' AND device_type='mobile' "
...
def read_checks(path: Path, clean: bool, sample: int | None) -> dict:
    ...
    if path.suffix == ".parquet":
        ...
        table = TRANSFORM.format(source=f"read_csv('{path}', header=false, skip=1, ignore_errors=true, all_varchar=true)",
                                 IOS_WEBKIT=IOS_WEBKIT, IOS_TOKEN=IOS_TOKEN,
                                 ANDROID_TOKEN=ANDROID_TOKEN, DESKTOP_OS_MARKER=DESKTOP_OS_MARKER)
    else:
...
def clean(input_path: Path, output_path: Path, sample: int | None) -> None:
    ...
    sql = f"COPY ({TRANSFORM.format(source=source, IOS_WEBKIT=IOS_WEBKIT, IOS_TOKEN=IOS_TOKEN, ANDROID_TOKEN=ANDROID_TOKEN, DESKTOP_OS_MARKER=DESKTOP_OS_MARKER)}) TO '{output_path}' (FORMAT PARQUET, COMPRESSION ZSTD)"
    con.execute(sql)
    con.close()
